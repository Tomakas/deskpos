...
/settings/register   → ScreenRegisterSettings (Režim, Prodej, Periferie) — vyžaduje settings_register.*
...
/kds                 → ScreenKds (kuchyňský displej, touch-optimized grid, status bumping) — vyžaduje orders.view
/vouchers            → ScreenVouchers (správa voucherů) — vyžaduje vouchers.view
/statistics          → ScreenStatistics (7 tabů: Dashboard, Receipts, Sales, Orders, Shifts, Z-Reports, Tips) — vyžaduje stats.*
/data                → ScreenData (3 taby: Export, Import, Backup — stubs) — vyžaduje data.*
/reservations        → ScreenReservations (seznam rezervací s date range filtrem, section filtrem, tabulkou, bar chart a Gantt chart) — vyžaduje venue.reservations_view
...
**Auth guard:** Router čeká na `appInitProvider`. Nepřihlášený uživatel je přesměrován na `/login`. Pokud neexistuje firma, přesměrování na `/onboarding`. Po přihlášení se z auth/onboarding stránek přesměruje na `/bills` (gastro mód) nebo `/sell` (retail mód).

**Permission guard:** Každá chráněná routa používá `hasAnyPermissionInGroupProvider` — uživatel se na obrazovku dostane, pokud má **alespoň jedno** oprávnění z příslušné skupiny. Routy `/settings/*` vyžadují odpovídající `settings_company.*` / `settings_venue.*` / `settings_register.*`. `/catalog` vyžaduje `products.*`, `/inventory` vyžaduje `stock.*`, `/vouchers` vyžaduje `vouchers.view`, `/statistics` vyžaduje `stats.*`. Routa `/orders` vyžaduje konkrétní `orders.view`. Route `/kds` vyžaduje `orders.view` (režim se volí při přihlášení, ale přístup je chráněn permission guardem). Route `/customer-display` a `/display-code` nemají permission guard (veřejný displej). Bez oprávnění se uživatel přesměruje na `/bills`.
...
│   ├── widgets/                       # PosTable<T>, PosTableToolbar, PosDialogShell, PosDialogActions, PosDialogTheme, PosNumpad, LockOverlay, InactivityDetector, PosColorPalette, PairingConfirmationListener, PosDateRangeSelector, HighlightedText, BusinessTypeSelector, VoidQuantityDialog
...
│   ├── printing/                      # PdfFontLoader, ReceiptData, ReceiptPdfBuilder, ZReportPdfBuilder, InventoryPdfBuilder, VoucherPdfBuilder, StockDocumentPdfBuilder, PrintingService
│   ├── routing/                       # GoRouter + auth guard (app_router.dart)
│   ├── network/                       # Supabase konfigurace (URL, anon key)
│   ├── sync/                          # Sync engine
│   │   ├── sync_service.dart          # Pull (30s interval, 42 tabulek, paginovaný po 1000 řádcích) + mergeRow (LWW) + tableDependencyOrder
...
│   ├── bills/                         # Přehled účtů
│   │   ├── screens/                   # ScreenBills, ScreenReservations
│   │   ├── widgets/                   # DialogBillDetail, DialogNewBill, DialogPayment,
│   │   │                              # DialogOpeningCash, DialogClosingSession,
│   │   │                              # DialogCashMovement, DialogCashJournal, DialogCashTender,
...
│   │   │                              # DialogReceiptPreview, DialogReservationEdit, FloorMapView
...
│   │                                  # DialogCustomerCredit, DialogCustomerTransactions
│   ├── inventory/                     # ScreenInventory (3 taby: Zásoby, Doklady, Pohyby),
│   │                                  # DialogStockDocument, DialogStockDocumentDetail, DialogInventoryType, DialogInventory, DialogInventoryResult
...
│   │                                  # ScreenRegisterSettings (3 taby: Režim, Prodej, Periferie),
│   │                                  # RegistersTab (CRUD pokladen v ScreenVenueSettings),
│   │                                  # ScreenCloudAuth (embedded widget v CloudTab, bez vlastní route),
│   │                                  # DialogGridEditor, DialogAutoArrange, FiscalTab (stub)
│   ├── data/                          # ScreenData (3 taby: Export, Import, Backup — stubs)
...
│   ├── statistics/                    # ScreenStatistics (7 tabů: Dashboard, Receipts, Sales, Orders, Shifts, Z-Reports, Tips),
│   │                                  # DashboardTab, DashboardSummaryCards, DashboardRevenueChart,
│   │                                  # DashboardDonutCharts, DashboardTopProducts, DashboardHeatmap, DashboardData,
│   │                                  # DialogShiftEdit
...
#### PosTable<T>
Generický unifikovaný tabulkový widget používaný napříč celou aplikací (33 použití v 23 souborech).
...
- **Text overflow:** `_wrapColumn` obaluje buňky do `DefaultTextStyle.merge(overflow: TextOverflow.ellipsis, maxLines: 1)` — automatický ellipsis pro všechny Text widgety uvnitř buněk
...
- **`PosNumpad`** — sdílený numpad se dvěma velikostmi (large/compact), konfigurovatelnými tlačítky (clear, dot, bottomLeftChild). Použito v 15 souborech / 16 instancích (login, lock, sell numpad ×2, opening cash, closing session, cash movement, loyalty, credit, discount, change total, voucher create, voucher redeem, cash tender, display code, void quantity)

Migrace: 31 dialog souborů celkem, z toho 25 migrovaných na sdílené widgety, 4 ponechané s vlastním layoutem (bill_detail, payment, grid_editor, cash_journal), 2 přidané později (dialog_shift_edit, void_quantity_dialog).
...
Databáze se ukládá v adresáři Application Support (`getApplicationSupportDirectory()` z `path_provider`):

```
<applicationSupportDirectory>/maty_database.sqlite
```

Cesta se skládá pomocí `path` package (`p.join(dir.path, 'maty_database.sqlite')`). Na macOS typicky `~/Library/Application Support/com.example.maty/`, na Android `data/data/<package>/files/`.
...
##### Aktivní tabulky (45) — registrované v @DriftDatabase
...
| **order_items** | id (T), company_id →companies, order_id →orders, item_id →items, item_name (T), quantity (R), unit (T — UnitType, default ks), voucher_discount (I, default 0), sale_price_att (I), sale_tax_rate_att (I), sale_tax_amount (I), discount (I, default 0), discount_type (T? — DiscountType), notes (T?), status (T — PrepStatus), prep_started_at (D?), ready_at (D?), delivered_at (D?) |
...
| Tabulka | Sloupce |
|---------|---------|
| **companies** | id (T), name (T), status (T — CompanyStatus), business_id (T?), address (T?), phone (T?), email (T?), vat_number (T?), country (T?), city (T?), postal_code (T?), timezone (T?), business_type (T? — BusinessType), default_currency_id →currencies, auth_user_id (T — uuid na Supabase, FK na auth.users), is_demo (B, default false), demo_expires_at (DT?) |
...
| Tabulka | Sloupce |
|---------|---------|
| **tables** | id (T), company_id →companies, section_id →sections?, table_name (T), capacity (I, default 0), is_active (B, default true), grid_row (I, default 0), grid_col (I, default 0), grid_width (I, default 3), grid_height (I, default 3), shape (T — TableShape, default rectangle), color (T?), font_size (I?), fill_style (I, default 1), border_style (I, default 1) |
| **map_elements** | id (T), company_id →companies, section_id →sections (nullable), grid_row (I, default 0), grid_col (I, default 0), grid_width (I, default 2), grid_height (I, default 2), label (T, nullable), color (T, nullable — hex #RRGGBB), shape (T — TableShape, default rectangle), font_size (I?), fill_style (I, default 1), border_style (I, default 1) |
...
| **customer_transactions** | id (T), company_id →companies, customer_id →customers, points_change (I), credit_change (I), order_id →orders?, processed_by_user_id →users, reference (T?), note (T?) |
...
| **stock_levels** | id (T), company_id →companies, warehouse_id →warehouses, item_id →items, quantity (R, default 0.0), min_quantity (R?) |
...
| **stock_movements** | id (T), company_id →companies, stock_document_id →stock_documents?, bill_id →bills?, item_id →items, quantity (R), purchase_price (I?), direction (T — StockMovementDirection), purchase_price_strategy (T? — PurchasePriceStrategy per-item override) |
...
- `stock_movements` — `stock_document_id` nullable: null pro automatické prodejní odpisy, nastaveno pro ruční doklady. `bill_id` nullable: nastaveno pro prodejní odpisy (vazba na účet), null pro dokladové pohyby
...
| Tabulka | Sloupce |
|---------|---------|
| **layout_items** | id (T), company_id →companies, register_id →registers, page (I, default 0), grid_row (I), grid_col (I), type (T — LayoutItemType), item_id →items?, category_id →categories?, label (T?), color (T?) |
...
- `color` — volitelná custom barva tlačítka (nullable). Formát: solid hex `#E53935` nebo gradient preset `linear:{angle}:{color1},{color2}` (např. `linear:135:#E53935,#FF5722`). Detekce: `startsWith('linear:')` → gradient, jinak solid. 12 předdefinovaných gradient presetů v `PosColorPalette`.
...
| Tabulka | Důvod |
|---------|-------|
| audit_log | Audit trail, write-only (zapisuje Ingest Edge Function). RLS zapnuto, SELECT policy `audit_log_select_own` pro `authenticated` (company-scoped). Zápis pouze přes Edge Functions (service_role). Sloupce: id (bigint, auto-generated PK), created_at (timestamptz, default now()), auth_user_id (uuid, NOT NULL), company_id (text?), table_name (text), entity_id (text), operation (text), idempotency_key (text?), status (text), error_message (text?). |
| seed_demo_data | Lookup tabulka pro demo data generátor (`create_demo_company` RPC). Sloupce: id (serial), locale (text), mode (text), entity_type (text), ref (text), data (jsonb), sort_order (integer). **RLS zapnuto, bez policies** — přístupná pouze přes service_role v `create_demo_company` SECURITY DEFINER funkci. |
| demo_log | Log demo company vytváření. RLS zapnuto, bez policies (přístup pouze přes service_role). Sloupce: id (bigint, auto PK), created_at (timestamptz), auth_user_id (uuid), company_id (text), locale (text), mode (text), status (text), error_message (text), started_at (timestamptz), finished_at (timestamptz), duration_ms (integer), ip_address (text), user_agent (text). |
...
- **Globální read-only**: `currencies`, `roles`, `permissions`, `role_permissions` — SELECT policy s `qual = true` pro roli `authenticated` (anonymní uživatelé mají přístup díky anonymous sign-in, který jim přiděluje `authenticated` JWT)
- `display_devices` má navíc SELECT, INSERT a UPDATE RLS politiky (company-scoped) pro přímé zápisy mimo ingest Edge Function (pairing flow)
- Sync tabulky vyžadují authenticated + company-scope policy
- **Anonymní přístup:** RLS policies používají roli `authenticated`, která zahrnuje i anonymní uživatele (Supabase anonymous sign-in přiděluje `authenticated` JWT). Toto je záměrné — demo firmy využívají anonymní přihlášení.
...
| `NegativeStockPolicy` | `CompanySettingsModel`, `ItemModel` (nullable) | allow, warn, block — company-level default + optional per-item override (null = inherit company) |
| `SellMode` | `RegisterModel` | gastro, retail — PG enum (od migrace 20260227_002). UI label: cs „Gastro" / „Maloobchod", en „Gastro" / „Retail" |
| `BusinessType` | `CompanyModel` | restaurant, bar, cafe, canteen, bistro, bakery, foodTruck, grocery, clothing, electronics, generalStore, florist, hairdresser, beautySalon, fitness, autoService, other — typ podnikání firmy, volitelný (nullable) |
...
> **Supabase PG enumy:** Na serveru existuje 26 PG enumů — 24 doménových (všechny výše uvedené včetně SellMode, NegativeStockPolicy a BusinessType) + 2 systémové: `sync_operation` (insert, update, delete) a `sync_status` (pending, processing, completed, failed) pro tabulku `sync_queue`. SellMode je PG enum (od migrace 20260227_002). Dart má 24 enumů (+ companion `BusinessCategory` enum pro UI kategorizaci BusinessType).
...
**38 company-scoped tabulek** se synchronizuje přes Push/Realtime/Pull. **4 globální tabulky** (currencies, roles, permissions, role_permissions) se pouze pullují (30s polling, bez broadcast triggerů).
...
- Stuck processing watchdog: 60s timeout auto-reset `_isProcessing` flagu (bezpečnost proti zamrznutí)

### Ingest Edge Function

Všechny outbox zápisy procházejí přes Supabase Edge Function `ingest` (`supabase/functions/ingest/index.ts`). `OutboxProcessor` nevolá Supabase tabulky přímo — místo toho posílá payload do EF přes `_supabaseClient.functions.invoke('ingest', ...)`. ALLOWED_TABLES obsahuje 38 tabulek (tabulka `companies` + 37 company-scoped tabulek; 42 doménových minus 4 globální server-owned: currencies, roles, permissions, role_permissions, které se seedují migrací; audit_log je write-only server-side).
...
**Architektura:**
- Server-side AFTER INSERT OR UPDATE triggers (`trg_{table}_broadcast`) na 38 company-scoped tabulkách
- Trigger funkce `broadcast_sync_change()` (37 tabulek s `company_id`) a `broadcast_company_sync_change()` (tabulka `companies` s `id`) volají `realtime.send()` s `to_jsonb(NEW)` do kanálu `sync:{company_id}`
...
- Migrace `20260222_001_add_broadcast_triggers.sql` vytvoří trigger funkce a triggery na 38 tabulkách
...
- `pullAll()` iteruje všech 42 tabulek v FK-respektujícím pořadí (`tableDependencyOrder`) — rodiče před dětmi
...
PostgreSQL BEFORE UPDATE trigger `trg_{table}_lww` s funkcí `enforce_lww` je aktivní na všech 42 doménových tabulkách (38 company-scoped + 4 globální: currencies, roles, permissions, role_permissions). Systémové tabulky (audit_log, sync_queue) LWW trigger nemají. Porovnává `client_updated_at` — pokud je příchozí timestamp starší než existující záznam, vyhodí výjimku `LWW_CONFLICT` (P0001). Ingest Edge Function tento error zachytí a vrátí klientovi `error_type: "lww_conflict"`.
...
- FIFO je zachováno
- **DB stuck reset** (processing > 5 min dle `processedAt` timestampu): `resetStuck()` jednorázově při startu `SyncLifecycleManager` resetuje stuck záznamy na `pending`. Startup sekvence: `resetStuck()` → `resetFailed()` → `deleteCompleted()` → initial push → drain loop
- **Reset failed**: `OutboxProcessor.start()` volá `resetFailed()` pro opětovný pokus dříve selhané entries
...
`roles` (4), `permissions` (105), `role_permissions`, `currencies` jsou **globální** (bez `company_id`):
...
4. `syncing` — Loading: „Synchronizace dat..." → `syncService.pullAll(companyId)` — stáhne všech 42 tabulek
...
-- Triggers on 38 tables call realtime.send() to push changes to sync:{company_id} channel.
...
-- 7. Permissions 105 (permissions_113 migration + stock_view_split + permission_cleanup + catalog_cleanup):
--    - Replaces 16 seed permissions with 105 granular permissions in 17 groups
--    - 260 role_permission assignments (helper: 15, operator: 55, manager: 85, admin: 105)
...
Bill totaly se přepočítávají **po každé změně** (createOrder, cancelOrder, voidOrder). Výpočet zahrnuje pouze aktivní položky (ne voided):
...
**Slevy (od Etapy 3.2):** 2 úrovně — položka (`order_items.discount` + `discount_type`) a účet (`bills.discount_amount` + `discount_type`). `DiscountType` enum: `absolute` (v haléřích) nebo `percent` (v setinách procenta, 10000 = 100%). UI: `DialogDiscount` s přepínačem Kč/%, numpadem a náhledem efektivní slevy. Kontextový titulek: „Sleva na celý účet" při aplikaci na bill, „Sleva na položku" při aplikaci na item. Slevy na úrovni objednávky (Order) neexistují.
...
Při vytvoření firmy (onboarding) se seedují **6 výchozích platebních metod**:
...
| Vouchery | `voucher` | Platba vouchery |
| Stravenky | `mealTicket` | Platba stravenkami |
...
stateDiagram-v2
    ...
    paid --> opened: reopenBill() [orders.reopen]
    cancelled --> opened: reopenBill() [orders.reopen]
    refunded --> opened: reopenBill() [orders.reopen]
```
...
> **Poznámka:** Stavy `paid`, `cancelled` a `refunded` jsou podmíněně terminální — lze je znovu otevřít přes `reopenBill()` s oprávněním `orders.reopen`.
...
stateDiagram-v2
    ...
    created --> voided: cancelOrder() / voidOrder()
...
|--------|-------|---------------|
| `created` | Objednávka vytvořena | `ready`, `voided` |
...
| `voided` | Stornováno (cancel i void vedou na `voided`) | (finální stav) |

> **Pozn.:** `markReady()` a `markDelivered()` delegují na `updateStatus()`, který nemá guard na zdrojový stav — přechody jsou vynucovány UI vrstvou (KDS/Orders screen), ne repository vrstvou. `cancelOrder()` je alias pro `voidOrder()` — obě vedou na `PrepStatus.voided`.
...
Pravidla (vyhodnocují se v pořadí, `activeItems` = items kde status ∉ {voided}):

1. `activeItems` prázdné (všechny voided) → Order = `voided`
2. Všechny `activeItems` delivered → Order = `delivered`
3. Všechny `activeItems` ∈ {ready, delivered} → Order = `ready`
4. Jinak (všechny `activeItems` created) → Order = `created`
...
| Aspekt | Rozhodnutí |
|--------|------------|
| **Bill číslo** | `B{register_number}-{counter:4d}` (např. `B1-0001`) — per-day reset, 4 cifry. Fallback bez registru: `B-{counter:4d}`. |
...
**Příznak:** `is_takeaway = true` v tabulce bills. V UI se zobrazuje jako "Rychlý účet". Při vytvoření se automaticky nastaví `section_id` na výchozí sekci (`isDefault=true`). Stejně tak konverze rychlého prodeje na běžný účet ("Uložit") resolvuje výchozí sekci.
...
sequenceDiagram
    ...
    Note over UI: 3 tlačítka: "Zrušit", "Uložit", "Zaplatit"

    Note over UI: Košík může obsahovat separátory → splitCartIntoGroups() rozdělí na více objednávek
    UI->>BR: createBill(companyId, userId, sectionId: default, isTakeaway: true, numberOfGuests: 1)
    BR->>DB: INSERT bills
    DB-->>BR: BillModel
    BR-->>UI: Success(bill)

    UI->>OR: createOrderWithItems(companyId, billId, items) — per group
    ...
    alt Platba úspěšná
        PAY-->>UI: true
        alt Gastro mode
            Note over UI: context.pop() — navigace zpět na /bills
        else Retail mode
            Note over UI: _resetForNextSale() — vyčistí košík, zůstane na ScreenSell
        end
        Note over UI: Customer display: thank-you message
    else Platba zrušena
        PAY-->>UI: false/null
...
- 3 tlačítka: **"Zrušit"** (storno), **"Uložit"** (`_convertToBill` — vytvoří běžný účet s `isTakeaway: false` na výchozí sekci, vytvoří objednávky, přepočítá totaly, naviguje zpět bez platby), **"Potvrdit platbu"** (vytvoří účet + otevře platební dialog)
- Po zrušení platby se účet automaticky stornuje
- V přehledu účtů se zobrazuje jako "Rychlý účet"
- **Retail mode:** Po úspěšné platbě `_resetForNextSale()` vyčistí košík a zůstane na ScreenSell (gastro mode naviguje zpět na /bills)
- **Cart separátory:** Košík lze rozdělit na více objednávek pomocí `_CartSeparator` objektů → `_splitCartIntoGroups()` před odesláním
...
Cancel i void vedou na stejný stav `voided` — `cancelOrder()` je alias pro `voidOrder()`. Obě metody nastaví status na `PrepStatus.voided` a spustí stock reversal.
...
- Pokud bill měl voucher (`voucherId != null`): repository nuluje `voucherDiscountAmount` a `voucherId` v bill záznamu; volající (UI vrstva — `DialogBillDetail`) zodpovídá za volání `VoucherRepository.unredeem()` před `cancelBill()` (počet uses se spočítá z aktivních položek pomocí `VoucherDiscountCalculator`)
...
- **cancelOrder:** alias pro `voidOrder()` — `status` musí být `created` nebo `ready` → všechny items → `voided`, order → `voided`
...
- Po stornování se **Bill totals přepočítají** (odečtou se voided items)

| Akce | Bill.status | Bill.totals |
|------|-------------|-------------|
| Void jedné order | Zůstává `opened` | Přepočítají se |
| Void všech orders | Zůstává `opened` (prázdný bill povolen) | 0 |
| cancelBill | → `cancelled` | Vynulovány (subtotal, total, slevy, rounding) + orders voided |
...
- **Business:** createDocument (transakce: insert document + movements + adjust stock_levels + update purchase_price dle strategie per položka, enqueue všech dotčených entit), createInventoryDocument (difference-based — porovná aktuální vs zadané množství, vytvoří korekční movements), generateDocumentNumber (R-0001/W-0001/I-0001/C-0001)
...
| Permission | 105 | Viz [Katalog oprávnění](#katalog-oprávnění-105) — 17 skupin |
| Role | 4 | helper, operator, manager, admin |
| RolePermission | 260 | helper: 15, operator: 55, manager: 85, admin: 105 |
| PaymentMethod | 6 | Viz [Platební metody](#platební-metody), vč. Zákaznický kredit (credit), Vouchery (voucher) a Stravenky (mealTicket) |
...
| Category | 0 (7 s demo) | S `withTestData` gastro: Hlavní jídla, Předkrmy, Dezerty, Nealko, Pivo, Víno, Ostatní. Retail: Potraviny, Nápoje, Drogerie, Domácnost, Ostatní |
| Item | 0 (18 s demo gastro) | S `withTestData` gastro: 4 hlavní jídla, 2 předkrmy, 2 dezerty, 3 nealko, 3 piva, 2 vína, 2 ostatní (kávy). Vše typ `product`, žádné recepty/varianty/modifikátory. Retail: 15 položek (5 kategorií) |
| ModifierGroup | 0 | Seed nevytváří modifikátory |
| ModifierGroupItem | 0 | Seed nevytváří modifikátory |
| ItemModifierGroup | 0 | Seed nevytváří modifikátory |
...
| UserPermission | 105 | Všech 105 oprávnění, granted_by: admin user ID (self-grant při onboardingu) |
...
4. `SyncService.pullAll(companyId)` stáhne data firmy v pořadí FK závislostí (42 tabulek):
   1. Currencies, Companies, CompanySettings, Roles, Permissions, RolePermissions
   2. Sections, TaxRates, PaymentMethods, CompanyCurrencies, Categories, Users, UserPermissions
   ...
   6. RegisterSessions, SessionCurrencyCash, CashMovements, Shifts, CustomerTransactions, Vouchers
...
┌─────────────────────────────────────────────────────┐
│  permissions (katalog)                              │
│  105 položek v 17 skupinách, read-only, seed lokálně│
└─────────────────────┬───────────────────────────────┘
                      │
┌─────────────────────▼───────────────────────────────┐
│  role_permissions (šablony)                         │
│  Vazba role → permission, read-only                 │
│  admin: 105, manager: 85, operator: 55, helper: 15  │
└─────────────────────┬───────────────────────────────┘
...
|------|-------------|:---------:|-------|
| `helper` | Pomocník / Číšník | 15 | Základní obsluha — objednávky, platby, vidí jen své věci |
| `operator` | Směnový vedoucí | 55 | Řídí směnu — storna, refundace, slevy (limitované), pokladní operace, statistiky (session) |
| `manager` | Manažer | 85 | Řídí provoz — slevy (neomezené), katalog, sklad, statistiky (historie), zaměstnanci, nastavení provozovny |
| `admin` | Administrátor / Majitel | 105 | Plný přístup — systém, daně, data, role, destruktivní akce |

### Katalog oprávnění (105)
...
| **Supabase trigger** | `guard_last_admin` | `BEFORE UPDATE` (`trg_users_guard_last_admin`) + `BEFORE DELETE` (`trg_users_guard_last_admin_delete`) na `users`, `ERRCODE 23514`. **Nasazen** (migrace `20260228_001_deploy_guard_last_admin`). |
...
Layout: **Left: Expanded(flex: 4), Right: fixed 290px collapsible AnimatedContainer**
...
│                               Posl.obj: 11:35            │
│┌────────────────────────────────────────┬───────────────┐│
││   Sumář / Historie objednávek (toggle) │  ZÁKAZNÍK     ││
││ 12:21 2ks Pivo 0,5l     110 Kč  ● ⋮  │  STŮL         ││
││ 12:21 1ks Cappuccino      65 Kč  ● ⋮  │  SLOUČIT      ││
││ 12:41 2ks Zákusek        130 Kč  ● ⋮  │  ROZDĚLIT     ││
││                                        │  SLEVA        ││
││                                        │  VOUCHER      ││
│└────────────────────────────────────────┴───────────────┘│
│  [ZAVŘÍT] [TISK] [STORNO] [ZAPLATIT] [OBJEDNAT]         │
└──────────────────────────────────────────────────────────┘
```
...
**Pravý sloupec (100px):** 6 permission-gated tlačítek — Zákazník (orders.assign_customer, jen pro otevřené → DialogCustomerSearch), Stůl (orders.transfer, přesun na jiný stůl), Sloučit (orders.merge), Rozdělit (orders.split), Sleva (discounts.apply_bill|apply_bill_limited, toggle: apply/remove — pokud účet má slevu, zobrazí „Zrušit slevu" s potvrzením), Voucher (vouchers.redeem, toggle: apply/remove — pokud účet má voucher, zobrazí „Zrušit voucher" s potvrzením a odebrání per-item voucherDiscount). Sloučit/Rozdělit/Stůl/Sleva/Voucher aktivní jen pro otevřené účty. Pozn.: Sumář/Historie je přepínací header v centrálním panelu, Tisk je ve footeru, Loyalty info se zobrazuje jako text v header oblasti.
**Bottom:** Podmíněný footer dle stavu účtu:
  - Vždy: ZAVŘÍT (OutlinedButton)
  - Vždy (conditional): TISK (printing.receipt|printing.reprint, tonal)
  - Otevřený bill: + STORNO (orders.void_bill, destructive outlined, s potvrzením → cancelBill) + ZAPLATIT (payments.accept, zelená, jen pokud totalGross > 0 → DialogPayment) + OBJEDNAT (modrá → `/sell/{billId}`)
  - Zaplacený bill: + REFUND (payments.refund, oranžová, s potvrzením → refundBill) + ZNOVU OTEVŘÍT (orders.reopen, warning). Klik na položku → refund per-item (s potvrzením).
  - Cancelled/Refunded: + ZNOVU OTEVŘÍT (orders.reopen, warning)
...
| STORNO | E2 | Storno účtu (cancelBill; prázdný účet bez objednávek → okamžité storno bez potvrzení; účet s objednávkami → potvrzovací dialog) |
| ZAVŘÍT | E2 | Zavře dialog (Navigator.pop) |
| REFUND | E3.2 | Refund celého účtu (jen pro paid bill, s potvrzením) |
| SLEVA | E3.2 | Toggle: pokud účet nemá slevu → DialogDiscount (s kontextovým titulem „Sleva na celý účet" / „Sleva na položku"); pokud účet má slevu → potvrzovací dialog → odebrání slevy (updateDiscount na 0) |
...
| VOUCHER | E3.7 | Toggle: pokud účet nemá voucher → DialogVoucherRedeem (numpad pro zadání číselného kódu `XXXX-XXXX` → validace → aplikace slevy); pokud účet má voucher → potvrzovací dialog → odebrání voucheru (unredeem uses, clearVoucherDiscounts, removeVoucher) |
| TISK | E3.7 | Tisk účtenky (DialogReceiptPreview — PDF generace a náhled) |

#### DialogPayment (platba účtu)

Layout: **3-sloupcový Expanded** (1:2:1 flex ratio, ConstrainedBox maxWidth: 500, maxHeight: ~240px)
...
│              │     1 250,00 Kč        │              │
│              │                        │              │
│  ZRUŠIT      │  Spropitné: 0 Kč       │              │
└──────────────┴────────────────────────┴──────────────┘
```

**Levý sloupec (flex: 1):** Akční tlačítka — Jiná měna (aktivní pokud existují cizí měny), Tisk účtenky (toggle print), Další akce (loyalty redeem, EET stub), ZRUŠIT (červená, zavře dialog — vrací `true` pokud byly provedeny platby).
**Centrum (flex: 2):** Nadpis "PLATBA", číslo účtu + stůl, seznam již provedených plateb (streamováno), zbývající částka (velký bold). Pokud je nastavena custom částka, zobrazuje se pod zbývající částkou. Při přeplatku zobrazuje spropitné. Tap na zbývající částku otevře DialogChangeTotalToPay pro custom amount.
**Pravý sloupec (flex: 1):** Platební metody — streamované z DB, pouze aktivní. Každá metoda = zelené tlačítko (48px). Click-to-pay: klik zaplatí `customAmount ?? remaining` danou metodou.
...
Layout: **Left cart: fixed 280px SizedBox, Right grid: Expanded**
...
**Pravý panel (Expanded) — Konfigurovatelný grid:**
...
| Daň. sazby | CRUD daňové sazby (název, typ, sazba %, výchozí) |
| Plat. metody | CRUD platební metody (název, typ dropdown, aktivní) |
...
| Periferie | Konfigurace periferních zařízení (stub — připraveno pro budoucí implementaci) |
...
│ ───────┼──────────┼────────────────┼───────────┼─────────│
│  R-0001│ Příjemka │ 10.02.2026     │ Pivovary  │ 5000.00│
...
**Tab Doklady:** DataTable se stock_documents. Sloupce: Číslo dokladu, Typ, Datum, Dodavatel (resolved přes supplier stream), Poznámka, Celkem. Tap na řádek → DialogStockDocumentDetail (hlavička dokladu + tabulka pohybů + tlačítko Tisk).
...
**Filtry (spodní lišta):** 4 filtry: Vytvořené / Hotové / Doručené / Stornované (voided). Default: created+ready. FilterChip row s Expanded.
...
**Přístup:** Route `/kds`, volba režimu na login obrazovce (POS/KDS radio) nebo přes ScreenRegisterSettings. Vyžaduje `orders.view` permission guard.
...
- Směny — tabulka `shifts` **implementována** (Drift + model + repository + sync enqueue + pull + Supabase tabulka s RLS). UI: Shifts tab v ScreenStatistics (přehled všech směn s datem, obsluhou, přihlášením/odhlášením, trváním; filtr dle data; probíhající směny zvýrazněny).
- Z-report — denní uzávěrka s detailním souhrnem. **Implementováno:** Z-Reports tab v ScreenStatistics + `DialogZReport`. Per-register i venue-wide Z-report s per-register breakdowns (Milník 3.9).
