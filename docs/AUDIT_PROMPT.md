...
**Overengineering pravidlo:** Každá nová abstrakce, wrapper, helper nebo layout struktura musí být odůvodněná. Pokud existuje jednodušší řešení se stejným výsledkem (méně vrstev, méně kódu, méně indirection), je to nález závažnosti NÍZKÉ (nebo STŘEDNÍ pokud zvyšuje složitost údržby).
...
- [ ] **Overengineering** — jsou nové abstrakce/helpery v repository/mapper/service vrstvě nutné? Nebylo jednodušší rozšířit existující metodu místo nové? Existují wrappery/indirection vrstvy, které nepřidávají hodnotu?
...
Checklist:
- [ ] **Overengineering** — jsou nové widgety/wrappery/layout struktury přiměřené? Existuje jednodušší řešení (méně FittedBox, LayoutBuilder, Flexible vrstev) se stejným výsledkem? Nepřidávají se globální wrappery kvůli jednomu dialogu? Neovlivňují nežádoucně ostatní dialogy?
...
- [ ] **Overengineering** — přidává se validace/error handling pro scénáře co nemohou nastat? Jsou nové helpery/utility jen pro jedno použití? Existují parametry/abstrakce přidané „pro budoucí potřebu" bez aktuálního využití?
...
**Overengineering** (zbytečné abstrakce, wrappery, layout vrstvy, indirection)

```
### [NÍZKÉ/STŘEDNÍ] Název nálezu
**Soubor:** `cesta/soubor.dart:řádek`
**Problém:** Co je zbytečně složité.
**Jednodušší alternativa:** Jak to zjednodušit.
```
